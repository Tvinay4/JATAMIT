...
const ReusableCard = ({ icon, title, description, color }) => (
  ...
  >
    <CardContent>
      {icon && <Box sx={{ fontSize: 48, mb: 2, color: primaryColor }}>{icon}</Box>}
...
const LandingAboutUsPage = () => {
  ...
  const testimonials = [
    ...
    },
    {
      ...
      avatar: "https://images.pexels.com/photos/1181686/pexels-photo-1181686.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=100&w=100",
      rating: 5,
    },
    {
      name: "Sonia Verma",
    ...
    },
    {
      ...
      avatar: "https://images.pexels.com/photos/91227/pexels-photo-91227.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=100&w=100",
      rating: 5,
    },
  ];
 ...
 const dataCards = [
  { title: "Revenue", icon: <MonetizationOn sx={{ color: "#FF6B6B", fontSize: 40 }} />, value: "$120K" },
  { title: "Users", icon: <People sx={{ color: "#4ECDC4", fontSize: 40 }} />, value: "1.2K" },
  { title: "Growth", icon: <TrendingUp sx={{ color: "#FFD93D", fontSize: 40 }} />, value: "35%" },
  { title: "Business", icon: <Business sx={{ color: "#6A4C93", fontSize: 40 }} />, value: "24 Partners" },
  { title: "Transactions", icon: <MonetizationOn sx={{ color: "#FFA500", fontSize: 40 }} />, value: "15K+" },
  { title: "Support Tickets", icon: <People sx={{ color: "#00CED1", fontSize: 40 }} />, value: "120 Resolved" },
  { title: "Active Clients", icon: <People sx={{ color: "#FF8C42", fontSize: 40 }} />, value: "350+" },
  { title: "Projects Completed", icon: <TrendingUp sx={{ color: "#6A4C93", fontSize: 40 }} />, value: "85%" },
];
...
const advantages = [
  { 
    title: "Fast & Secure", 
    description: "Our system ensures ultra-fast transactions with end-to-end encryption.",
  ...
  return (
    <Box sx={{ overflowX: "hidden" }}>
      ...
      <Box sx={{ py: 7, bgcolor: creamBg }}>
           <Typography
    variant="h3"
    sx={{ color: accentColor, fontWeight: "bold",textAlign:"center",mb:4 }}
  >
  ...
  <Typography variant="body1" sx={{ color: "#555", mb: 2, lineHeight: 1.6 }}>
    Jatmit Payment Systems is transforming the way businesses and individuals handle digital transactions. With a commitment to innovation and security, we provide seamless financial solutions tailored to the needs of the modern economy.
  </Typography>

  <Typography variant="body1" sx={{ color: "#555", mb: 2, lineHeight: 1.6 }}>
    Our platform integrates advanced technologies to ensure fast, reliable, and secure payments. Whether it's domestic transfers, international remittances, or merchant services, Jatmit empowers you to transact confidently.
  </Typography>

 

  <Typography variant="body1" sx={{ color: "#555", mb: 3, lineHeight: 1.6 }}>
    With 24/7 support, cutting-edge analytics, and global reach, Jatmit Payment Systems is your trusted partner in the journey toward a cashless, digital-first future.
  </Typography>
</Grid>
            ...
            <Grid item xs={12} md={6}>
              <Box
                ...
                src={aboutus}
                alt="About Us"
                sx={{ width: "100%", borderRadius: 6, boxShadow: "0px 8px 20px rgba(0,0,0,0.08)" }}
              />
            </Grid>
          </Grid>
        </Container>
          ...
          <Grid container spacing={4}>
            {dataCards.map((card, idx) => (
              <Grid item xs={12} sm={6} md={3} key={idx}>
                <Card sx={{ backgroundColor: "#fff", borderRadius: 3, boxShadow: 3, p: 3, textAlign: "center" }}>
                  {card.icon}
                  <Typography variant="h6" sx={{ mt: 1 }}>{card.title}</Typography>
                  <Typography variant="subtitle1" color="#555">{card.value}</Typography>
                </Card>
              </Grid>
            ))}
          </Grid>
        ...
        <Container>
          ...
          <Slider {...sliderSettings}>
            {testimonials.map((item, idx) => (
              <Box key={idx} sx={{ px: 2 }}>
                <Card sx={{ p: 4, borderRadius: 6, boxShadow: "0 4px 20px rgba(0,0,0,0.05)", bgcolor: "#fff" }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    <Avatar src={item.avatar} alt={item.name} sx={{ width: 64, height: 64, mr: 2 }} />
                    <Box>
                      <Typography variant="subtitle1" sx={{ fontWeight: "bold" }}>{item.name}</Typography>
                      <Typography variant="body2" color="#555">{item.position}</Typography>
